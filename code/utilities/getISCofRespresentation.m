...
for frq = 1:length(cfg.frequencies)
    ...
    if cfg.permutationtest
        ...
        for cate_num = 1:numel(cfg.categories)
            ...
            stats.(frqBand).cluster.(category) = struct;
        end
    ...
    if cfg.plotting
        ...
        for icat=1:length(cfg.categories)
            ...
            if cfg.permutationtest
                ...
                if ~isempty(stats.(frqBand).cluster.(category).obs_cluster_stats)
                    for p=1:length(stats.(frqBand).cluster.(category).pvals_cluster)
                        if stats.(frqBand).cluster.(category).pvals_cluster(p) <= 0.05
                            idx = stats.(frqBand).cluster.(category).obs_clusters.PixelIdxList{p};
                            sigMat(idx)=1;
                        end
                    end
                end
        ...
        title(['Median inter-subject pattern correlation - ', (frqBand)]);
        legend(h);
        legend('boxoff');
        set(gca, 'box', 'off');
